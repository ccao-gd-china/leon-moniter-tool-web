...
@WebServlet("/cpl_exception_monitor_servlet")
public class cpl_exception_monitor_servlet extends HttpServlet {

  private static int count = 1;
  private CPLService cplService = new CPLService();

  private Logger logger = LoggerFactory.getLogger(cpl_exception_monitor_servlet.class);
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (queryString != null) {
      ...
      //} else {
      getServletContext().getRequestDispatcher("/WEB-INF/cpl_exception_monitor.html")
        .forward(req, resp);
    }
  }
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      if (!queryStr.equals("")) {
        ...
        purchaseLogs = cplService
          .get_all_cpl_exception_monitor_logs_by_period(response_code, reason_code, queryStr,
            cplQueryVo);
        recordsTotal = cplService
          .get_cpl_exception_monitor_count_by_period(response_code, reason_code, queryStr,
            cplQueryVo);
      } else {
        cplQueryVo = initCPLQueryVo2(req);
        purchaseLogs = cplService
          .get_all_cpl_exception_monitor_logs_by_period(response_code, reason_code, startDate,
            endDate, cplQueryVo);
        recordsTotal = cplService
          .get_cpl_exception_monitor_count_by_period(response_code, reason_code, startDate, endDate,
            cplQueryVo);
      }
      ...
      if (purchaseLogs != null && purchaseLogs.size() != 0) {
        ...
        jsonObject.put("status_code", response_code + "/" + reason_code);
        String error_description = cplService.get_error_description(response_code, reason_code);
