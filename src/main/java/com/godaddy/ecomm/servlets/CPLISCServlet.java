...
@WebServlet("/CPLISCServlet")
public class CPLISCServlet extends HttpServlet {

  private static int count = 1;
  private CPLService cplService = new CPLService();

  private Logger logger = LoggerFactory.getLogger(CPLISCServlet.class);
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (queryString != null) {
      ...
      resp.sendRedirect(req.getContextPath() + "/CPLISCServlet");
    } else {
      getServletContext().getRequestDispatcher("/WEB-INF/cplISCRefundMonitor.html")
          .forward(req, resp);
    }
  }
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    List<CPLISCRefundLog> purchaseLogs;
    int recordsTotal;
    int recordsFiltered;
    ...
    try {
      ...
      if (!queryStr.equals("")) {
        if (queryStr.contains("latest")) {
          ...
          purchaseLogs = cplService.getCPLISCRefundLogsByLatestSnapshotId(cplQueryVo);
          recordsTotal = cplService.getCPLISCRefundLogCountByLatestSnapshotId();
        } else {
          destroyQueryFields();
          cplQueryVo = initCPLQueryVo2(req);
          purchaseLogs = cplService.getCPLISCRefundLogsByPeriod(queryStr, cplQueryVo);
          recordsTotal = cplService.getCountByPeriod("CPLISCRefundLog", queryStr, cplQueryVo);
        }
      } else {
        cplQueryVo = initCPLQueryVo2(req);
        purchaseLogs = cplService.getALLCPLISCRefundLogs(cplQueryVo);
        recordsTotal = cplService.getCount("CPLISCRefundLog",cplQueryVo);
      }
